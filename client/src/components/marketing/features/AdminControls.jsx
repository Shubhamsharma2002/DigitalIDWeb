import Image from "next/image";
import { Images } from "../../../../constant/iamge";


export default function AdminControls() {
  return (
    <section className="px-5 py-10 bg-[#F8FAFF]">
      <div className="max-w-7xl mx-auto rounded-[2rem] bg-white border border-blue-100 shadow-[0_20px_60px_rgba(37,99,235,0.08)] p-6 md:p-10">
        <div className="grid lg:grid-cols-[0.8fr_1.2fr] gap-10 items-center">
          <div>
            <h2 className="text-3xl font-black text-[#0B1437] mb-8">
              Admin Controls 🏢
            </h2>

            <div className="space-y-8">
              {[
                ["☁️", "Bulk Import", "Hazaron users ko CSV se 1-click mein add karein."],
                ["🎨", "Organization Branding", "Logo, color theme aur identity customize karein."],
              ].map(([icon, title, desc]) => (
                <div key={title} className="flex gap-5 items-start">
                  <div className="h-16 w-16 rounded-2xl bg-violet-100 flex items-center justify-center text-3xl shadow-inner">
                    {icon}
                  </div>
                  <div>
                    <h3 className="text-xl font-black text-blue-600">{title}</h3>
                    <p className="mt-2 text-slate-600 leading-relaxed">{desc}</p>
                  </div>
                </div>
              ))}
            </div>
          </div>

          <div className="relative rounded-[1.5rem] bg-gradient-to-br from-blue-50 to-violet-50 p-4 border border-slate-100 overflow-hidden">
            <div className="absolute -top-16 -right-16 h-40 w-40 rounded-full bg-blue-300/30 blur-3xl" />
            <Image
              src={Images.DashboardPreview}
              alt="Admin dashboard preview"
              width={800}
              height={520}
              className="relative z-10 w-full h-auto rounded-2xl shadow-xl"
            />
          </div>
        </div>
      </div>
    </section>
  );
}